...
impl BiomeTree {
            ...
            Self::Branch { nodes, .. } => {
                ...
                for node in nodes {
                    ...
                    if distance > node_distance {
                        ...
                        let node2_distance = if std::ptr::eq(node, node2) {
                            node_distance
                        } else {
    ...
    fn get_squared_distance(&self, other: &[i64; 7]) -> i64 {
        ...
        parameters
            ...
            .zip(other)
            .map(|(bound, value)| {
                let distance = bound.get_distance(*value);
                distance * distance
            })
            .sum()
    }
...
mod test {
    ...
    use crate::{
        ...
        NOISE_ROUTER_ASTS, ProtoChunk, read_data_from_file,
    };
